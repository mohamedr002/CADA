...
def cross_domain_train(da_params, config, model, my_dataset, src_id, tgt_id, run_id):
    ...
    # optimizer
    discriminator_optim = torch.optim.AdamW(discriminator.parameters(), lr=da_params['lr'], betas=(0.5, 0.5))
    target_optim = torch.optim.AdamW(target_encoder.parameters(), lr=da_params['lr'], betas=(0.5, 0.5))
    ncl_optim = torch.optim.AdamW(comput_ncl.parameters(), lr=0.01, betas=(0.5, 0.5))
    if da_params['tensor_board']:
        comment = (f'/home/emad/Mohamed2/runs/model={config["model_name"]} Scenario={src_id} to {tgt_id}_wo_do')
    ...
    for epoch in range(1, da_params['epochs'][src_id] + 1):
        ...
        for _ in range(da_params['iterations']):  # , leave=False):
            ...
            for _ in range(da_params['k_clf']):
                ...
                total_loss = loss + 0*ncl_loss #0.2*ncl_loss 
        ...
        mean_tgt_loss = target_loss / (da_params['iterations'] * da_params['k_clf'])
        # tensorboard logging
        if da_params['tensor_board']:
            tb.add_scalar('Discriminator_loss', mean_loss, epoch)
            tb.add_scalar('NCL_loss', mean_ncl, epoch)
        ...
        print(f'Target_loss:{mean_tgt_loss} \t NCL_loss{mean_ncl}')
        if epoch % 1 == 0:
    ...
    if da_params['save']:
        torch.save(target_model.state_dict(), f'/home/emad/Mohamed2/trained_models/cross_domains/{src_id}_to_{tgt_id}_{run_id}_ncl_loss.pt')
